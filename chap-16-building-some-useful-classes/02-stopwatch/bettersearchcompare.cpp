...
int binary_search(const vector<int>& v, int seek)
{
    ...
    int last = v.size() - 1; // Initially the last element in vector
...
int main()
{
    ...
    // Search for all the lements in list using linear search
    timer.start();
    for (size_t i = 0; i < SIZE; i++)
    {
    ...
    timer.stop();
    cout << "Linear elapsed: " << timer.elapsed() << " seconds\n";
    ...
    // Print the elapsed time
    timer.stop();
    cout << "Binary elapsed: " << timer.elapsed() << " seconds\n";
}